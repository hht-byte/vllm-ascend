...
# Access through the module so the opt-in PTO implementation from #8872 can
# still replace the fallback without affecting the default AscendC path.
...
class AscendGatedDeltaNetAttention(GatedDeltaNetAttention):
    @staticmethod
    def _get_fused_chunk_op(
        state_dtype: torch.dtype,
        activation_dtype: torch.dtype = torch.bfloat16,
    ):
        """Select the accuracy-preserving fused GDN prefill implementation.

        Prefer the vLLM-Ascend custom op backported from #12607 because it
        preserves both BF16 and FP32 recurrent-state dtypes. Newer torch_npu
        builds may provide the operator directly; that path is only selected
        for BF16 state because its public contract does not preserve FP32.
        Returning ``None`` keeps the existing Triton/PTO path as a safe
        compatibility fallback.
        """
        if activation_dtype != torch.bfloat16 or state_dtype not in (torch.bfloat16, torch.float32):
            return None

        custom_namespace = getattr(torch.ops, "_C_ascend", None)
        custom_op = getattr(custom_namespace, "npu_chunk_gated_delta_rule", None)
        if callable(custom_op):
            return "custom", custom_op

        builtin_op = getattr(torch_npu, "npu_chunk_gated_delta_rule", None)
        if state_dtype == torch.bfloat16 and callable(builtin_op):
            return "torch_npu", builtin_op
        return None
    ...
    def _run_fused_chunk(
        ...
        """Run fused GDN prefill using the TND interface from #12607/#12647."""
        q_tnd = l2norm_fwd(q.squeeze(0)).contiguous()
        k_tnd = l2norm_fwd(k.squeeze(0)).contiguous()
        v_tnd = v.squeeze(0).contiguous()
        beta_tnd = beta.squeeze(0).to(v_tnd.dtype).contiguous()
        g_tnd = g.squeeze(0).to(torch.float32).contiguous()
    ...
    def _forward_core(
        ...
        if attn_metadata.num_prefills > 0:
            fused_op = (
                self._get_fused_chunk_op(ssm_state.dtype, query_non_spec.dtype)
                if get_pcp_group().world_size == 1
                else None
            )
            if fused_op is not None:
                # The ACLNN op consumes the native state layout
                # [B, Nv, Dv, Dk], so no pre/post transpose is required.
                fallback_meta = attn_metadata.non_spec_prefill_fallback_meta
                if fallback_meta is None:
                    raise RuntimeError("Expected non-spec prefill metadata for fused GDN prefill path.")
                initial_state_full = ssm_state[non_spec_state_indices_tensor].contiguous()
                clear_ssm_states(initial_state_full, has_initial_state)
                actual_seq_lengths = fallback_meta.actual_seq_lengths
                non_empty_indices = fallback_meta.non_empty_indices
                ...
                core_attn_out_non_spec, last_recurrent_state = self._run_fused_chunk(
                    *fused_op,
                    q=query_non_spec,
                    k=key_non_spec,
                    v=value_non_spec,
