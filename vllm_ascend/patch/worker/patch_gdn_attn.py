...
def _patched_build_prefill(
    ...
    actual_seq_lengths = (
        attn_metadata.non_spec_query_start_loc[1:] - attn_metadata.non_spec_query_start_loc[:-1]
    ).to(torch.int32).contiguous()
    non_empty_indices = None
    if bool(torch.any(non_spec_query_start_loc_cpu[1:] == non_spec_query_start_loc_cpu[:-1])):
        non_empty_indices = torch.nonzero(actual_seq_lengths, as_tuple=False).squeeze(-1)
