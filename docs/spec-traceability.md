...
| 15 | 310P 上每请求实际 Encoder/KV 命中、Audio Tower/LLM prefill 时间分段、TTFT、尾字时延、P50/P95、NPU 内存及指标端点可用性 | `test_310p_cache_reuse_matches_full_recompute_after_lifecycle_events` | `python benchmarks/benchmark_310p.py --help`; `msprof --help`; `python -m pytest -m npu tests/integration/test_310p_equivalence.py -vv` | 目标机待验收 |
...
| 17.5 | 目标机 Audio Tower/LLM prefill 分段、外部真实峰值 NPU 内存、无 profiler 正式性能矩阵及数值 sidecar 归档 | `test_310p_cache_reuse_matches_full_recompute_after_lifecycle_events` | `msprof --help`; `npu-smi info`; `python benchmarks/benchmark_310p.py --validate-numeric-sidecar <sidecar>` | 目标机待验收 |
