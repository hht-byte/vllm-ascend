                                                     ...
                                                     aclOpExecutor **executor)
{
    ...
    CHECK_RET(ret == ACLNN_SUCCESS, ret);
    std::unique_ptr<int64_t, decltype(&std::free)> stateShapeGuard(stateShapeDims, &std::free);
    ret = aclGetViewStrides(initialState, &stateStrideDims, &stateStrideDimsNum);
    CHECK_RET(ret == ACLNN_SUCCESS, ret);
    std::unique_ptr<int64_t, decltype(&std::free)> stateStrideGuard(stateStrideDims, &std::free);
